...
def var_sigma_2(sigma, alpha, beta, w1, w2, p):
    return 0.5*p*(sp.erf((1-w1)*alpha/(sigma*np.sqrt(2)))+sp.erf((1+w1)*alpha/(sigma*np.sqrt(2))))+0.5*(1-p)*(sp.erf((1-w2)*beta/(sigma*np.sqrt(2)))+sp.erf((1+w2)*beta/(sigma*np.sqrt(2))))
